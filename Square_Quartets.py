...
EXSquare=np.dot(SX1,SX2)+np.dot(SY1,SY2)+np.dot(SZ1,SZ2)+np.dot(SX2,SX3)+np.dot(SY2,SY3)+np.dot(SZ2,SZ3)+np.dot(SX3,SX4)+np.dot(SY3,SY4)+np.dot(SZ3,SZ4)+np.dot(SX4,SX1)+np.dot(SY4,SY1)+np.dot(SZ4,SZ1)
...
def EvsH_cgs(g,theta,phi,Jex,Field,NP):
   ...
   for i in range(NP+2):
       v=LA.eigvals((muB_cgs/kB_cgs) * Zeeman(g,theta,phi) * i /NP*Field*1000 + 2*Jex*EXSquare)
       v=v.real
       v=np.sort(v)
...
plt.text(0, np.max(Energy[:,Energy.shape[1]-1]*2/3) , 'AF Square Quartets, Spin = %s ' % Spin , fontsize=10)
...
plt.text(np.max(M1[:,0])*1/3, np.max(M1[:,1])/8, 'AF Square Quartets, Spin = %s ' % Spin , fontsize=10)

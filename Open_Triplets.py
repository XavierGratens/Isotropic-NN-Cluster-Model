...
T2=time.time()-T1
M1=M_OpenTriplets(g,0,0,Jex,60,1000,Temp1)
T3=time.time()-T2
...
plt.text(0, np.max(Energy[:,Energy.shape[1]-1]*2/3) , 'AF Open Triplets, Spin = %s ' % Spin , fontsize=10)
...
plt.text(np.max(M1[:,0])*1/2, np.max(M1[:,1])/8, 'AF Open Triplets, Spin = %s ' % Spin , fontsize=10)
...
np.savetxt("M_OT_S32.txt", M1 ,delimiter=' ' )
plt.show()
